...
"""
================================================================================
Visit Tunisia — Scraper Nabeul (FINAL — 20 spots de qualité)
================================================================================
Combine OSM + fallback manuel de spots touristiques réels.
Seuil adaptatif : noms propres acceptés même sans Wikidata.
================================================================================
"""
...
GOVERNORAT = "Nabeul"
BBOX = {"s": 36.20, "w": 10.42, "n": 37.00, "e": 11.20}
LAT_MIN, LAT_MAX = 36.25, 36.95
LON_MIN, LON_MAX = 10.45, 11.15
...
# Noms génériques à bannir (même avec Wikidata)
NAME_BLACKLIST = [
    ...
    r"^maison\s+de\s+\w+$",  # "Maison de houssine" mais pas "Maison de la Culture"
    r"^résidence\s+\w+$",     # "Résidence Mahmoud" mais pas "Résidence Hammamet"
]

CITY_BLACKLIST = [
    "tunis", "تونس", "carthage", "قرطاج", "sidi bou said", "سيدي بوسعيد",
    "la marsa", "المرسى", "le kram", "الكرم", "gammarth", "قمرت",
    "sousse", "سوسة", "monastir", "المنستير", "mahdia", "المهدية",
...
# -----------------------------------------------------------------------------
# FALLBACK MANUEL — Spots touristiques réels de Nabeul
# -----------------------------------------------------------------------------

MANUAL_SPOTS = [
    # Hammamet
    ("Médina de Hammamet", "SITE_TOURISTIQUE", ["CULTUREL"], 36.4000, 10.6167,
     "La médina de Hammamet est une ancienne cité fortifiée datant du XVe siècle, célèbre pour ses ruelles blanches et ses souks.", "attraction"),
    ("Fort de Hammamet", "SITE_TOURISTIQUE", ["CULTUREL"], 36.4010, 10.6150,
     "Le fort de Hammamet est une forteresse ottomane dominant la mer Méditerranée, construite au XIIIe siècle.", "castle"),
    ("Plage de Hammamet", "SITE_TOURISTIQUE", ["BALNEAIRE"], 36.4050, 10.6200,
     "La plage de Hammamet est l'une des plus célèbres de Tunisie, s'étendant sur des kilomètres de sable fin.", "beach"),
    ("Yasmin Hammamet", "SITE_TOURISTIQUE", ["BALNEAIRE"], 36.3800, 10.5400,
     "Yasmin Hammamet est une marina moderne avec des hôtels de luxe, restaurants et boutiques au bord de la Méditerranée.", "beach_resort"),
    ("Flipper Aquapark", "ACTIVITE", ["AVENTURE", "BALNEAIRE"], 36.4100, 10.6000,
     "Flipper Aquapark est un parc aquatique offrant de nombreux toboggans et piscines pour toute la famille.", "water_park"),
    
    # Nabeul centre
    ("Musée de Nabeul", "SITE_TOURISTIQUE", ["CULTUREL"], 36.4507, 10.7362,
     "Le musée de Nabeul est un musée archéologique présentant des collections romaines et une célèbre mosaïque.", "museum"),
    ("Souk de la poterie de Nabeul", "SITE_TOURISTIQUE", ["CULTUREL"], 36.4550, 10.7350,
     "Le souk de la poterie est le cœur artisanal de Nabeul, réputé pour sa céramique colorée et son travail de l'argile.", "attraction"),
    ("Plage de Nabeul", "SITE_TOURISTIQUE", ["BALNEAIRE"], 36.4600, 10.7500,
     "La plage de Nabeul offre une longue étendue de sable doré idéale pour la baignade et les sports nautiques.", "beach"),
    
    # Kelibia
    ("Fort de Kelibia", "SITE_TOURISTIQUE", ["CULTUREL"], 36.8500, 11.0900,
     "Le fort de Kelibia est une impressionnante forteresse punique puis byzantine dominant le cap Bon.", "castle"),
    ("Plage La Mansoura (Kelibia)", "SITE_TOURISTIQUE", ["BALNEAIRE"], 36.8550, 11.1000,
     "La plage La Mansoura est une magnifique étendue de sable fin à Kelibia, prisée des amateurs de windsurf.", "beach"),
    ("Cap Bon", "SITE_TOURISTIQUE", ["BALNEAIRE", "AVENTURE"], 36.9000, 11.0500,
     "Le Cap Bon est l'extrémité nord-est de la Tunisie, offrant des falaises spectaculaires et des vues panoramiques.", "cape"),
    
    # Autres
    ("Plage de Raf Raf", "SITE_TOURISTIQUE", ["BALNEAIRE"], 37.1856, 10.2172,
     "La plage de Raf Raf est une plage sauvage et préservée du nord du Cap Bon, entourée de collines verdoyantes.", "beach"),
    ("Korba", "SITE_TOURISTIQUE", ["BALNEAIRE"], 36.5667, 10.8667,
     "Korba est une ville côtière du Cap Bon connue pour ses plages et son agriculture florissante.", "beach"),
    ("Plage de Sidi Ali El Mekki", "SITE_TOURISTIQUE", ["BALNEAIRE"], 37.1703, 10.2535,
     "La plage de Sidi Ali El Mekki est une station balnéaire tranquille près de Ghar El Melh.", "beach"),
    ("Plage de Dar Chaabane", "SITE_TOURISTIQUE", ["BALNEAIRE"], 36.4667, 10.7500,
     "La plage de Dar Chaabane est une plage familiale proche de Nabeul, idéale pour une journée détente.", "beach"),
    ("Grottes d'El Haouaria", "SITE_TOURISTIQUE", ["ECOLOGIQUE", "AVENTURE"], 37.0500, 11.0167,
     "Les grottes d'El Haouaria sont d'anciennes carrières romaines creusées dans la falaise, offrant un paysage unique.", "cave"),
    ("Plage de Tazarka", "SITE_TOURISTIQUE", ["BALNEAIRE"], 36.5500, 10.9167,
     "La plage de Tazarka est une station balnéaire paisible entre Nabeul et Kelibia.", "beach"),
    ("Mosquée de Hammamet", "SITE_TOURISTIQUE", ["RELIGIEUX", "CULTUREL"], 36.4020, 10.6180,
     "La grande mosquée de Hammamet est un édifice religieux historique situé au cœur de la médina.", "mosque"),
    ("Restaurant Le Barberousse", "RESTAURANT", ["GASTRONOMIQUE"], 36.3949, 10.6127,
     "Le Barberousse est un restaurant réputé de la médina de Hammamet, spécialisé dans les fruits de mer.", "restaurant"),
    ("Hôtel Royal Kenz", "HEBERGEMENT", ["BALNEAIRE"], 36.3950, 10.6100,
     "L'hôtel Royal Kenz est un établissement 4 étoiles situé en bord de mer à Hammamet.", "hotel"),
]


def inject_manual_spots():
    """Convertit les spots manuels en format OSM-like."""
    elements = []
...
def is_in_nabeul(lat, lon, tags, name):
    ...
    for black in CITY_BLACKLIST:
        if black in city:
            if any(w in name_lower for w in ["hammamet", "nabeul", "kelibia", "korba"]):
                continue
            return False
    return True
...
def clean_osm_elements(elements):
    ...
    for elem in elements:
        ...
        if not is_in_nabeul(lat, lon, tags, name):
            continue
        ...
        # Seuil adaptatif : noms propres (score > 20) OK, noms douteux (score < 25) rejet
        if quality_score < 25:
        ...
        # Description pré-enrichie si présente dans OSM
...
def merge_sources(osm_destinations, manual_elements):
    """Fusionne OSM + manuel, priorise OSM si doublon."""
    ...
    for m in manual_cleaned:
        ...
        if key not in osm_names:
            # Injecter une description de qualité si manuelle
...
def enrich_wikidata(destinations):
    ...
    for dest in destinations:
        ...
        if wid and wid in wiki_data:
            w = wiki_data[wid]
            # Ne pas écraser une description manuelle de qualité
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            if any(p in extract.lower() for p in bad_patterns):
                return ""
            # Filtrer les descriptions hors sujet
...
def enrich_wikipedia(destinations):
    ...
    count = 0
    for dest in destinations:
        # Si déjà bonne description, skip
...
def enrich_images(destinations):
    ...
    for dest in destinations:
        ...
        if not img:
            img = search_commons_image(f"{dest['name']} Nabeul")
        if img:
            dest["image_url"] = img
            count += 1
        time.sleep(0.4)
