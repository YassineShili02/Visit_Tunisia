...
"""
================================================================================
Visit Tunisia — Scraper Sousse (FINAL — 20 spots de qualité)
================================================================================
Combine OSM + fallback manuel de spots touristiques réels du gouvernorat de Sousse.
Exclut Nabeul (nord), Monastir (sud), et Mahdia.
================================================================================
"""
...
# BBOX du gouvernorat de Sousse (Sahel)
# Nord: 36.15 (sous Nabeul/Bouficha), Sud: 35.70 (au-dessus de Monastir)
# Ouest: 10.20, Est: 10.70 (côte)
BBOX = {
    "s": 35.70,
    "w": 10.20,
    "n": 36.15,
...
LAT_MIN, LAT_MAX = 35.65, 36.20
LON_MIN, LON_MAX = 10.15, 10.75
...
CITY_BLACKLIST = [
    ...
    "monastir", "المنستير", "mahdia", "المهدية",
...
# -----------------------------------------------------------------------------
# FALLBACK MANUEL — Spots touristiques réels de Sousse
# -----------------------------------------------------------------------------

MANUAL_SPOTS = [
    # Sousse centre (Médina, culturel)
    ("Médina de Sousse", "SITE_TOURISTIQUE", ["CULTUREL"], 35.8256, 10.6360,
     "La médina de Sousse est un site du patrimoine mondial de l'UNESCO, célèbre pour ses remparts, sa kasbah et ses ruelles historiques.", "attraction"),
    ("Ribat de Sousse", "SITE_TOURISTIQUE", ["CULTUREL"], 35.8270, 10.6380,
     "Le Ribat de Sousse est une forteresse monastique du VIIIe siècle offrant une vue panoramique sur la médina et la mer.", "castle"),
    ("Grande Mosquée de Sousse", "SITE_TOURISTIQUE", ["RELIGIEUX", "CULTUREL"], 35.8260, 10.6370,
     "La Grande Mosquée de Sousse est un édifice religieux historique datant du IXe siècle, au cœur de la médina.", "mosque"),
    ("Musée Archéologique de Sousse", "SITE_TOURISTIQUE", ["CULTUREL"], 35.8240, 10.6350,
     "Le musée archéologique de Sousse abrite une exceptionnelle collection de mosaïques romaines et de sculptures antiques.", "museum"),
    ("Bab El Bhar", "SITE_TOURISTIQUE", ["CULTUREL"], 35.8230, 10.6340,
     "Bab El Bhar est la principale porte de la médina de Sousse, donnant directement sur le front de mer.", "city_gate"),
    
    # Port El Kantaoui & plages (Balnéaire)
    ("Port El Kantaoui", "SITE_TOURISTIQUE", ["BALNEAIRE", "CULTUREL"], 35.8910, 10.5940,
     "Le port El Kantaoui est une marina luxueuse avec ses maisons blanches à coupoles, ses restaurants et ses boutiques.", "attraction"),
    ("Plage de Kantaoui", "SITE_TOURISTIQUE", ["BALNEAIRE"], 35.8950, 10.5900,
     "La plage de Kantaoui est une magnifique étendue de sable fin bordant la station balnéaire haut de gamme.", "beach"),
    ("Plage de Sousse", "SITE_TOURISTIQUE", ["BALNEAIRE"], 35.8300, 10.6400,
     "La plage de Sousse s'étend sur plusieurs kilomètres le long de la corniche, idéale pour la baignade et les sports nautiques.", "beach"),
    ("Plage Boujaâfar", "SITE_TOURISTIQUE", ["BALNEAIRE"], 35.8350, 10.6450,
     "La plage Boujaâfar est la plage la plus animée de Sousse, située en plein centre-ville.", "beach"),
    
    # Environ de Sousse
    ("Hergla", "SITE_TOURISTIQUE", ["BALNEAIRE"], 36.0300, 10.5100,
     "Hergla est un village de pêcheurs pittoresque au nord de Sousse, réputé pour sa plage tranquille et ses restaurants de poisson.", "beach"),
    ("Plage d'Hergla", "SITE_TOURISTIQUE", ["BALNEAIRE"], 36.0320, 10.5120,
     "La plage d'Hergla offre un cadre naturel préservé avec ses eaux cristallines et ses falaises.", "beach"),
    ("Kalâa Kebira", "SITE_TOURISTIQUE", ["CULTUREL"], 35.8667, 10.5333,
     "Kalâa Kebira est une ville historique connue pour son agriculture et ses traditions artisanales.", "attraction"),
    ("Enfidha", "SITE_TOURISTIQUE", ["BALNEAIRE"], 35.9333, 10.3833,
     "Enfidha est une ville côtière du Sahel connue pour ses plages et son aéroport international.", "beach"),
    ("Plage d'Enfidha", "SITE_TOURISTIQUE", ["BALNEAIRE"], 35.9400, 10.3700,
     "La plage d'Enfidha est une longue étendue de sable doré peu fréquentée, idéale pour le farniente.", "beach"),
    
    # Parcs & loisirs
    ("Parc Farhat Hached", "SITE_TOURISTIQUE", ["ECOLOGIQUE", "CULTUREL"], 35.8280, 10.6420,
     "Le parc Farhat Hached est le principal espace vert de Sousse, parfait pour une promenade en famille.", "attraction"),
    
    # Gastronomie & Hébergement
    ("Restaurant Le Méditerranée", "RESTAURANT", ["GASTRONOMIQUE"], 35.8320, 10.6380,
     "Le Méditerranée est un restaurant réputé de Sousse spécialisé dans la cuisine tunisienne et les fruits de mer.", "restaurant"),
    ("Hôtel Marhaba Palace", "HEBERGEMENT", ["BALNEAIRE"], 35.8400, 10.6350,
     "L'hôtel Marhaba Palace est un établissement 5 étoiles situé en bord de mer avec vue sur la médina.", "hotel"),
    ("Hôtel El Kantaoui Center", "HEBERGEMENT", ["BALNEAIRE"], 35.8900, 10.5950,
     "L'hôtel El Kantaoui Center est un complexe hôtelier moderne à proximité de la marina.", "hotel"),
    ("Café Sidi Bou Hdid", "RESTAURANT", ["GASTRONOMIQUE", "CULTUREL"], 35.8265, 10.6375,
     "Le café Sidi Bou Hdid est un café historique perché sur les remparts de la médina, offrant une vue imprenable.", "cafe"),
]
...
def is_in_region(lat, lon, tags, name):
    ...
    for black in CITY_BLACKLIST:
        if black in city:
            if any(w in name_lower for w in ["sousse", "kantaoui", "hergla", "enfidha"]):
                continue
            return False
    return True
...
def enrich_images(destinations):
    ...
    for dest in destinations:
        ...
        if not img:
            img = search_commons_image(f"{dest['name']} Sousse")
        if img:
            dest["image_url"] = img
            count += 1
        time.sleep(0.4)
