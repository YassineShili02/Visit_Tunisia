...
"""
================================================================================
Visit Tunisia — Scraper Medenine (FINAL — 20 spots de qualité)
================================================================================
Combine OSM + fallback manuel de spots touristiques réels du gouvernorat de Médenine.
Inclut Djerba, Zarzis et Ben Guerdane. Exclut Tataouine, Gabès et le reste du pays.
================================================================================
"""
...
# BBOX du gouvernorat de Médenine (Sud-Est + Djerba)
# Nord: 33.95 (nord Djerba), Sud: 33.05 (limite Tataouine / Ben Guerdane sud)
# Ouest: 10.30 (à l'est de Gabès), Est: 11.25 (frontière Libye côté Ben Guerdane)
BBOX = {
    "s": 33.05,
    "w": 10.30,
    "n": 33.95,
...
LAT_MIN, LAT_MAX = 33.00, 34.00
LON_MIN, LON_MAX = 10.25, 11.30
...
CITY_BLACKLIST = [
    ...
    "kef", "الكاف", "tozeur", "توزر", "gafsa", "قفصة",
    "tataouine", "تطاوين", "chenini", "cheneni", "douiret", "guermessa", "bir lahmar", "remada",
    "gabes", "قابس", "kebili", "قبلي", "matmata", "مطماطة",
...
# -----------------------------------------------------------------------------
# FALLBACK MANUEL — Spots touristiques réels de Médenine (Djerba, Zarzis, etc.)
# -----------------------------------------------------------------------------

MANUAL_SPOTS = [
    # Médenine centre — Culturel
    ("Ksar de Médenine", "SITE_TOURISTIQUE", ["CULTUREL"], 33.3547, 10.5050,
     "Le Ksar de Médenine est un impressionnant grenier fortifié du XIXe siècle avec ses ghorfas en terre ocre, emblème de la région.", "castle"),
    ("Musée de Médenine", "SITE_TOURISTIQUE", ["CULTUREL"], 33.3500, 10.5000,
     "Le musée de Médenine retrace l'histoire berbère, la vie nomade et les traditions du Dahar et du Jérid.", "museum"),
    
    # Djerba — Culturel & Religieux
    ("Houmt Souk", "SITE_TOURISTIQUE", ["CULTUREL", "BALNEAIRE"], 33.8775, 10.8592,
     "Houmt Souk est la capitale de Djerba, célèbre pour sa médina animée, ses souks colorés et son port de pêche.", "attraction"),
    ("Synagogue La Ghriba", "SITE_TOURISTIQUE", ["RELIGIEUX", "CULTUREL"], 33.8120, 10.8570,
     "La Ghriba est la plus ancienne synagogue d'Afrique, haut lieu de pèlerinage juif et joyau architectural de Djerba.", "synagogue"),
    ("Musée de Guellala", "SITE_TOURISTIQUE", ["CULTUREL"], 33.7560, 10.9910,
     "Le musée de Guellala présente l'histoire millénaire de Djerba à travers des scènes de la vie traditionnelle en terre cuite.", "museum"),
    ("Borj El Kebir", "SITE_TOURISTIQUE", ["CULTUREL"], 33.8800, 10.8550,
     "Borj El Kebir est une forteresse espagnole du XVIe siècle dominant le port de Houmt Souk, offrant une vue panoramique.", "castle"),
    
    # Djerba — Balnéaire
    ("Plage de Sidi Mahrez", "SITE_TOURISTIQUE", ["BALNEAIRE"], 33.8250, 10.9900,
     "La plage de Sidi Mahrez est la plus célèbre de Djerba, avec son sable fin et ses eaux turquoise idéales pour la baignade.", "beach"),
    ("Plage de Seguia", "SITE_TOURISTIQUE", ["BALNEAIRE"], 33.7850, 11.0250,
     "La plage de Seguia est une magnifique étendue sauvage à l'est de Djerba, prisée pour son calme et ses paysages.", "beach"),
    ("Zone Touristique de Midoun", "SITE_TOURISTIQUE", ["BALNEAIRE"], 33.8000, 10.9800,
     "La zone touristique de Midoun concentre les plus beaux hôtels de Djerba, ses spas et ses animations nocturnes.", "beach_resort"),
    ("Phare de Taguermess", "SITE_TOURISTIQUE", ["BALNEAIRE", "CULTUREL"], 33.7200, 10.9500,
     "Le phare de Taguermess marque la pointe sud-ouest de Djerba, offrant des vues spectaculaires sur le coucher de soleil.", "attraction"),
    ("Ajim", "SITE_TOURISTIQUE", ["BALNEAIRE", "CULTUREL"], 33.7230, 10.7530,
     "Ajim est un port pittoresque à l'ouest de Djerba, célèbre pour ses pêcheurs et son marché aux poissons.", "attraction"),
    
    # Zarzis & Ben Guerdane
    ("Plage de Zarzis", "SITE_TOURISTIQUE", ["BALNEAIRE"], 33.5000, 11.1000,
     "La plage de Zarzis est une longue étendue de sable doré bordée de palmiers, idéale pour le farniente et les sports nautiques.", "beach"),
    ("Port de Zarzis", "SITE_TOURISTIQUE", ["BALNEAIRE", "CULTUREL"], 33.5100, 11.1200,
     "Le port de Zarzis est un centre de pêche actif et une marina offrant des excursions en mer vers les îles voisines.", "attraction"),
    ("Plage de Ben Guerdane", "SITE_TOURISTIQUE", ["BALNEAIRE"], 33.1500, 11.2167,
     "La plage de Ben Guerdane offre un cadre naturel préservé à proximité de la frontière libyenne.", "beach"),
    ("Marché de Ben Guerdane", "SITE_TOURISTIQUE", ["CULTUREL", "GASTRONOMIQUE"], 33.1400, 11.2100,
     "Le marché de Ben Guerdane est un carrefour commercial animé où se croisent les influences tunisienne et libyenne.", "attraction"),
    
    # Nature & Aventure
    ("Parc Djerba Explore", "ACTIVITE", ["AVENTURE"], 33.8300, 10.9200,
     "Le parc Djerba Explore abrite un village traditionnel, un musée de la crocodile et des animations culturelles.", "theme_park"),
    ("Îlot de la Flamme", "SITE_TOURISTIQUE", ["ECOLOGIQUE", "BALNEAIRE"], 33.4500, 11.1500,
     "L'îlot de la Flamme est une petite île préservée au large de Zarzis, refuge d'oiseaux et spot de snorkeling.", "attraction"),
    ("Dunes de Zarzis", "SITE_TOURISTIQUE", ["ECOLOGIQUE", "AVENTURE"], 33.4800, 11.1300,
     "Les dunes de Zarzis offrent des paysages désertiques en bord de mer, parfaits pour le quad et le sahara trekking.", "attraction"),
    
    # Gastronomie & Hébergement
    ("Restaurant Le Djerbien", "RESTAURANT", ["GASTRONOMIQUE"], 33.8750, 10.8600,
     "Le Djerbien est un restaurant réputé de Houmt Souk spécialisé dans la cuisine locale : couscous djerbien, poisson grillé et briks.", "restaurant"),
    ("Café des Nattes Houmt Souk", "RESTAURANT", ["GASTRONOMIQUE", "CULTUREL"], 33.8780, 10.8580,
     "Le Café des Nattes est une institution de la médina de Houmt Souk, idéal pour déguster un thé à la menthe en terrasse.", "cafe"),
    ("Hôtel Djerba Plaza", "HEBERGEMENT", ["BALNEAIRE"], 33.8100, 10.9750,
     "L'hôtel Djerba Plaza est un complexe hôtelier 4 étoiles situé en bord de mer à Midoun, avec piscine et animations.", "hotel"),
    ("Hôtel Ras El Ain Zarzis", "HEBERGEMENT", ["BALNEAIRE"], 33.5050, 11.1150,
     "L'hôtel Ras El Ain est un établissement en front de mer à Zarzis, parfait pour un séjour balnéaire au calme.", "hotel"),
]
...
def is_in_region(lat, lon, tags, name):
    ...
    for black in CITY_BLACKLIST:
        if black in city:
            if any(w in name_lower for w in ["medenine", "djerba", "jerba", "houmt souk", "zarzis", "ben guerdane", "midoun", "ajim", "guellala", "taguermess", "ksar", "ghriba"]):
                continue
            return False
    return True
...
def enrich_images(destinations):
    ...
    for dest in destinations:
        ...
        if not img:
            img = search_commons_image(f"{dest['name']} Medenine")
        if not img:
            img = search_commons_image(f"{dest['name']} Djerba")
        if img:
            dest["image_url"] = img
            count += 1
        time.sleep(0.4)
