...
"""
================================================================================
Visit Tunisia — Scraper Monastir (FINAL — 20 spots de qualité)
================================================================================
Combine OSM + fallback manuel de spots touristiques réels du gouvernorat de Monastir.
Exclut Sousse (nord), Mahdia (sud) et Nabeul.
================================================================================
"""
...
# BBOX du gouvernorat de Monastir (Sahel)
# Nord: 35.85 (sous Sousse), Sud: 35.55 (au-dessus de Mahdia)
# Ouest: 10.60, Est: 11.10 (côte)
BBOX = {
    "s": 35.55,
    "w": 10.60,
    "n": 35.85,
...
LAT_MIN, LAT_MAX = 35.50, 35.90
LON_MIN, LON_MAX = 10.55, 11.15
...
CITY_BLACKLIST = [
    ...
    "la goulette", "حلق الوادي", "bizerte", "بنزرت"
]
...
# -----------------------------------------------------------------------------
# FALLBACK MANUEL — Spots touristiques réels de Monastir
# -----------------------------------------------------------------------------

MANUAL_SPOTS = [
    # Monastir centre (Culturel & Religieux)
    ("Ribat de Monastir", "SITE_TOURISTIQUE", ["CULTUREL"], 35.7775, 10.8264,
     "Le Ribat de Monastir est une forteresse monastique du VIIIe siècle, célèbre pour sa tour de guet et ses vues panoramiques.", "castle"),
    ("Mausolée Habib Bourguiba", "SITE_TOURISTIQUE", ["CULTUREL", "RELIGIEUX"], 35.7760, 10.8280,
     "Le mausolée de Bourguiba est un monument imposant de marbre doré dédié au premier président de la Tunisie.", "monument"),
    ("Grande Mosquée de Monastir", "SITE_TOURISTIQUE", ["RELIGIEUX", "CULTUREL"], 35.7780, 10.8250,
     "La Grande Mosquée de Monastir est un édifice religieux historique au cœur de la médina.", "mosque"),
    ("Médina de Monastir", "SITE_TOURISTIQUE", ["CULTUREL"], 35.7770, 10.8270,
     "La médina de Monastir est un quartier historique aux ruelles pittoresques bordé par les remparts du Ribat.", "attraction"),
    ("Musée des Arts Islamiques de Monastir", "SITE_TOURISTIQUE", ["CULTUREL"], 35.7765, 10.8260,
     "Le musée abrite une collection unique de manuscrits, céramiques et textiles islamiques.", "museum"),
    ("Bab El Gharbi", "SITE_TOURISTIQUE", ["CULTUREL"], 35.7755, 10.8245,
     "Bab El Gharbi est une ancienne porte de la médina de Monastir, témoin de l'architecture défensive.", "city_gate"),
    
    # Marina & Plages (Balnéaire)
    ("Marina de Monastir", "SITE_TOURISTIQUE", ["BALNEAIRE", "CULTUREL"], 35.7800, 10.8350,
     "La marina de Monastir est un port de plaisance moderne avec restaurants, boutiques et activités nautiques.", "attraction"),
    ("Plage de Monastir", "SITE_TOURISTIQUE", ["BALNEAIRE"], 35.7820, 10.8400,
     "La plage de Monastir s'étend le long de la corniche, offrant du sable fin et des eaux claires.", "beach"),
    ("Plage de Skanès", "SITE_TOURISTIQUE", ["BALNEAIRE"], 35.7900, 10.8500,
     "La plage de Skanès est une magnifique étendue bordant les complexes hôteliers, idéale pour la baignade.", "beach"),
    ("Plage de Khniss", "SITE_TOURISTIQUE", ["BALNEAIRE"], 35.7500, 10.8100,
     "La plage de Khniss est une plage familiale tranquille au sud de Monastir.", "beach"),
    ("Plage de Sahline", "SITE_TOURISTIQUE", ["BALNEAIRE"], 35.7333, 10.8000,
     "La plage de Sahline est réputée pour son ambiance locale et ses eaux peu profondes.", "beach"),
    
    # Environ de Monastir
    ("Ksar Hellal", "SITE_TOURISTIQUE", ["CULTUREL"], 35.6500, 10.9000,
     "Ksar Hellal est une ville historique connue pour son artisanat textile et son rôle dans l'histoire politique tunisienne.", "attraction"),
    ("Moknine", "SITE_TOURISTIQUE", ["CULTUREL"], 35.6333, 10.9000,
     "Moknine est célèbre pour ses poteries traditionnelles et son artisanat de la terre cuite.", "attraction"),
    ("Téboulba", "SITE_TOURISTIQUE", ["BALNEAIRE"], 35.6333, 10.9500,
     "Téboulba est une ville côtière offrant des plages authentiques et un port de pêche animé.", "beach"),
    ("Plage de Téboulba", "SITE_TOURISTIQUE", ["BALNEAIRE"], 35.6350, 10.9550,
     "La plage de Téboulba est une étendue de sable doré prisée des locaux pour son calme.", "beach"),
    
    # Parcs & loisirs
    ("Parc du Ribat", "SITE_TOURISTIQUE", ["ECOLOGIQUE", "CULTUREL"], 35.7785, 10.8275,
     "Le parc du Ribat est un espace vert ombragé idéal pour une pause près de la forteresse.", "attraction"),
    
    # Gastronomie & Hébergement
    ("Restaurant Le Méditerranée Monastir", "RESTAURANT", ["GASTRONOMIQUE"], 35.7810, 10.8320,
     "Le Méditerranée est un restaurant réputé spécialisé dans les fruits de mer et la cuisine méditerranéenne.", "restaurant"),
    ("Café Sidi El Bahri", "RESTAURANT", ["GASTRONOMIQUE", "CULTUREL"], 35.7770, 10.8285,
     "Le café Sidi El Bahri est un établissement historique offrant une vue sur le port et la médina.", "cafe"),
    ("Hôtel Skanès Serail", "HEBERGEMENT", ["BALNEAIRE"], 35.7920, 10.8550,
     "L'hôtel Skanès Serail est un complexe hôtelier 4 étoiles en bord de mer avec de vastes jardins.", "hotel"),
    ("Hôtel Marina Palace", "HEBERGEMENT", ["BALNEAIRE"], 35.7815, 10.8360,
     "L'hôtel Marina Palace est un établissement moderne à proximité de la marina et de la plage.", "hotel"),
]
...
def is_in_region(lat, lon, tags, name):
    ...
    for black in CITY_BLACKLIST:
        if black in city:
            if any(w in name_lower for w in ["monastir", "skanes", "khniss", "sahline", "ksar hellal", "moknine", "teboulba"]):
                continue
            return False
    return True
...
def enrich_images(destinations):
    ...
    for dest in destinations:
        ...
        if not img:
            img = search_commons_image(f"{dest['name']} Monastir")
        if img:
            dest["image_url"] = img
            count += 1
        time.sleep(0.4)
